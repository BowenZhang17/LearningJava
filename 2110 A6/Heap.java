/** NetId(s): Bowen Zhang Time spent: 3 hours, 30 minutes.
 *
...
public class Heap<V> {
    ...
    public void insert(V v, double p) throws IllegalArgumentException {
       ...
       //Testing procedure test00Insert should work. Look at its specification.
    	...
    	VP insert1 = new VP(v,p);
    	b[size] = insert1;
    	map.put(insert1.value,size);
    	size++;
    	bubbleUp(size-1);
    ...
    void bubbleDown(int k) {
        ...
        while (c < this.size() && b[k].priority > b[c].priority) {
        	swap(k, c);
        	k = c;
        	c = smallerChildOf(k);
        }
    ...
    protected int smallerChildOf(int n) {
    	...
    	int leftChild = 2*n + 1;
    	if(rightChild >= this.size() || b[rightChild].priority > b[leftChild].priority) {
    		return leftChild;
    	}
		else {
		return rightChild;	
    ...
    public void changePriority(V v, double p) {
        ...
        if (!map.containsKey(v)) throw new IllegalArgumentException();
    	int heapIndex = map.get(v);
        b[heapIndex].priority = p;
        if (p < b[(heapIndex-1)/2].priority) bubbleUp(heapIndex);
        else bubbleDown(heapIndex);
